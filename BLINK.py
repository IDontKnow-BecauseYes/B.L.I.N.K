...
# Robust entry: fallback for non-interactive / missing-Streamlit environments
...
def get_logs():
    return pd.read_sql_query(
        "SELECT l.id, l.habit_id, l.ts, l.note, h.name as habit_name FROM logs l LEFT JOIN habits h ON h.id=l.habit_id ORDER BY ts DESC",
        conn
    )
...
def weekly_counts(df_logs, days=28):
    ...
    s = df.set_index('ts').groupby(pd.Grouper(freq='D')).size().reindex(rng, fill_value=0)
...
# --- Fallback behavior when Streamlit not installed ---
def run_fallback_if_needed():
    """
    Run a safe, non-interactive summary in sandboxed environments.
    If the environment is interactive, offer a simple CLI.
    This function never calls sys.exit() so it won't terminate the process unexpectedly.
    """
    if STREAMLIT_IMPORTED:
        # Streamlit is available — do not run fallback.
        return

    def print_header():
        print("B.L.I.N.K — Behavior Log (fallback mode)")
        print("Streamlit is not installed in this environment.")
        print("To run the full app with UI, install dependencies and run:")
        print("  pip install -r requirements.txt")
        print("  streamlit run blink_streamlit_app.py")
        print()
    ...
    def cli_summary():
        ...
        if dfh.empty:
            ...
            for _, r in dfh.iterrows():
                print(f"  {r['id']}: {r['name']} — {r['category'] or '-'} (meta {r['target']})")
        print()
        print(f"Total de registros: {len(dfl)}")
        if not dfl.empty:
            print(dfl[['ts','habit_name','note']].head(5).to_string(index=False))
        print()
        cur_streak, best = calc_streaks(dfl)
        print(f"Sequência atual (geral): {cur_streak}, melhor sequência: {best}")
        print()
        print("Note: interactive CLI disabled in this environment. To create habits or logs, run the app locally with Streamlit or deploy it where you can access the UI.")

    def interactive_cli_loop():
        # Only run this if the environment is interactive (TTYs available)
        def cli_list():
            dfh = get_habits()
            dfl = get_logs()
            print("Hábitos:")
            if dfh.empty:
                ...
                for _, r in dfh.iterrows():
                    print(f"  {r['id']}: {r['name']} — {r['category']} (meta {r['target']})")
            print()
            print(f"Últimos registros ({len(dfl)}):")
            if dfl.empty:
                print("  (nenhum)")
            else:
                print(dfl[['ts','habit_name','note']].to_string(index=False))

        def cli_create_habit():
            name = input('Nome do hábito: ').strip()
            if not name:
                print('Nome vazio — abortando')
                return
            category = input('Categoria (opcional): ').strip()
            try:
                target = int(input('Meta por dia [1]: ') or 1)
            except ValueError:
                target = 1
            color = input("Cor (hex) [#7c3aed]: ") or '#7c3aed'
            add_habit(name, category, target, color)
            print('Hábito criado')

        def cli_log():
            dfh = get_habits()
            if dfh.empty:
                print('Sem hábitos — crie um primeiro')
                return
            print('Escolha o id do hábito para registrar:')
            print(dfh[['id','name']].to_string(index=False))
            try:
                hid = int(input('id: ').strip())
            except Exception:
                print('id inválido')
                return
            note = input('Observação (opcional): ').strip()
            add_log(hid, datetime.utcnow().isoformat(), note)
            print('Registrado')

        def cli_help():
            print('Comandos disponíveis:')
            print('  list        — mostrar hábitos e últimos logs')
            print('  new         — criar hábito')
            print('  log         — registrar atividade')
            print('  exit        — sair')
        ...
        while True:
            try:
                ...
                print('\nI/O error or EOF reading input. Exiting interactive CLI.')
                break
            if cmd in ('exit','quit'):
                break
            if cmd == 'list':
    ...
    # Choose path based on interactivity
    if IS_INTERACTIVE:
        interactive_cli_loop()
    else:
        cli_summary()

# Only run fallback when executing the script directly
if __name__ == '__main__':
    run_fallback_if_needed()

# ---- Streamlit UI: only run if Streamlit is imported successfully ----
if STREAMLIT_IMPORTED:
    # Page config
    ...
    .header {display:flex;align-items:center;gap:18px;padding:18px;border-radius:14px;background: linear-gradient(180deg, rgba(255,255,255,0.03), rgba(255,255,255,0.01));box-shadow: 0 6px 18px rgba(2,6,23,0.6);} .h-title{font-size:22px;font-weight:700;margin:0} .h-sub{color:var(--muted);margin:0;font-size:13px}
    ...
    .confetti{position:fixed;left:0;top:0;width:100%;height:0;pointer-events:none;} .confetti span{position:absolute;top:-10vh;font-size:12px;opacity:0.9;animation:fall linear infinite}
    </style>
    """

    HERO_SVG = r'''<svg width="72" height="72" viewBox="0 0 72 72" fill="none" xmlns="http://www.w3.org/2000/svg"><rect width="72" height="72" rx="16" fill="url(#g)"/><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#7c3aed"/><stop offset="1" stop-color="#06b6d4"/></linearGradient></defs><g opacity="0.98"><path d="M36 18C29 18 24 23 24 30C24 40 36 54 36 54C36 54 48 40 48 30C48 23 43 18 36 18Z" fill="white" opacity="0.95"/></g></svg>'''
